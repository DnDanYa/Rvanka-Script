...
-- Цветовые настройки (по умолчанию)
local HUDColor = Color3.fromRGB(80, 80, 80)
local TextColor = Color3.fromRGB(255, 255, 255)
local HUDTransparency = 0.3
local OutlineColor = Color3.fromRGB(255, 0, 0) -- Цвет обводки по умолчанию
...
-- Создаем красивый HUD с серым фоном
...
-- Создаем элементы для выбора цвета
...
-- Функции для перемещения текста координат
...
-- Функция для флинг-атаки (Ultra режим)
local function UltraFling(TargetPlayer)
    ...
    -- Сохраняем позицию перед атакой
...
-- Создаем радужную обводку для персонажа
...
-- Телепорт и отбрасывание (Default режим)
...
RunService.Heartbeat:Connect(function()
    ...
    if CurrentTarget and CurrentTarget.Character then
        if CurrentMode == "Default" then
            ...
            -- Ultra режим: используем флинг-атаку
...
-- Обновляем обводку при появлении нового персонажа
